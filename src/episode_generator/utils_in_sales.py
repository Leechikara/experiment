...
def in_sales_controller(script, available_intent, grammar_p_dict, intent_p_dict, desired_entity):
    ...
    picked_intent = random_pick(list(intent_p_dict.keys()), list(intent_p_dict.values()), pick_num)
    if type(picked_intent) is not list:
        picked_intent = [picked_intent]

    # make topic coherent
    scene_dict = {'pay': [], 'express': []}
    for intent in picked_intent:
    ...
    for key in scene_dict.keys():
        ...
        picked_intent.extend(scene_content)
